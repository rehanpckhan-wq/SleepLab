...
export const InstallPWA: React.FC = () => {
  ...
  const renderModal = () => (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[9999]">
      <div className="bg-[var(--surface)] border border-[var(--border-default)] rounded-xl max-w-md w-full p-6 shadow-2xl relative font-sans space-y-4 animate-in fade-in zoom-in-95 duration-150">
        <button
        ...
        <div className="flex justify-end pt-2">
          <button
            onClick={() => setShowInstructions(false)}
            className="px-4 py-2 bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white text-xs font-sans font-medium rounded-md transition-colors shadow-sm cursor-pointer"
          >
            Got it
          </button>
        </div>
  ...
  return (
    <>
      ...
      {showInstructions && mounted && createPortal(renderModal(), document.body)}
    </>
  );
};
